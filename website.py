...
user_url = st.text_input('Enter the product URL')
options = ["Amazon", "Flipkart"]
user_platform = st.selectbox('Listing Source:', options)
    ...
    with st.sidebar:
        try:
            ...
            with open('reviews.txt', 'w', encoding='utf-8') as f:
                for line in all_detail_product_user[0][5]:
                    f.write(f"{line}\n")

            f.close()
            st.title("Your Product")
            st.markdown(f"<div style='text-align: center; padding: 10px;'><img src='{all_detail_product_user[0][7]}' style='width: 80%;'></div>",unsafe_allow_html=True)    
            st.markdown(f'<div style="text-align: center;"><a href="{all_detail_product_user[0][6]}" target="_blank">Go to product page</a>', unsafe_allow_html=True)   

            st.markdown(f"## {all_detail_product_user[0][0]}")
            st.markdown(f"### Price: {all_detail_product_user[0][1]}")
            if all_detail_product_user[0][9] == None:
                st.markdown(f"*Product Sales: Not Available*")
            else:
                st.markdown(f"*Product Sales: {all_detail_product_user[0][9]}*+ bought in the past month")            
            st.markdown(f"Average Rating on Website: {all_detail_product_user[0][10]}")
